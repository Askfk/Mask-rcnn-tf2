...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    # a = tf.range(probs.shape[0])
    # print('a: ', a.shape)
    # indices = tf.stack([tf.range(probs.shape[0]), class_ids], axis=1)
    indices = tf.stack([tf.range(config.POST_NMS_ROIS_TRAINING), class_ids], axis=1)
    ...
    refined_rois = utils_graph.clip_box_graph(refined_rois, window)
    ...
    def nms_keep_map(class_id):
        ...
        gap = tf.maximum(config.DETECTION_MAX_INSTANCES - tf.shape(class_keep)[0], 0)
    ...
    # Arrange outputs as [N, (y1, x1, y2, x2, class_id, socre)]
    # Coordinates are normalized
